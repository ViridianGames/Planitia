...
void Gui::LoadLegacyFile(const std::string& fileName)
{
	m_GuiElementList.clear();
	...
	getline(instream, line); // legacy font config line (ignored)
	...
	while (!instream.eof())
	{
		...
		case LEGACY_BUTTON:
		{
			...
			const string bitmap = ReadLegacyStringField(lineData);
			Texture* texture = LoadLegacyTexture(bitmap);
			if (texture)
			{
				AddIconButton(buttonId, texture, posX, posY, tileX, tileY, width, height,
					"", m_Font.get(), Color{ 255, 255, 255, 255 }, 1.0f, group, active != 0);
			}
			break;
		}
		...
		case LEGACY_SCROLLBAR:
		{
			...
			GuiScrollBar* scrollbar = AddScrollBar(id, height, posX, posY, width, height, true,
		...
		case LEGACY_TEXTAREA:
		{
			...
			const string text = ReadLegacyStringField(lineData);
			AddTextArea(id, m_Font.get(), text, posX, posY, width, height,
				Color{ static_cast<unsigned char>(r), static_cast<unsigned char>(g),
					static_cast<unsigned char>(b), static_cast<unsigned char>(a) },
		...
		case LEGACY_RADIOBUTTON:
		{
			...
			if (texture)
			{
				auto selected = make_shared<Sprite>(texture, tileX, tileY + height, width, height);
				auto unselected = make_shared<Sprite>(texture, tileX, tileY, width, height);
				auto hovered = make_shared<Sprite>(texture, tileX + width, tileY, width, height);
				GuiRadioButton* radioButton = AddRadioButton(buttonId, posX, posY, selected, unselected, hovered,
		...
		case LEGACY_BITMAP:
		{
			...
			if (texture)
			{
				auto sprite = make_shared<Sprite>(texture, 0, 0, texture->width, texture->height);
				AddSprite(id, posX, posY, sprite, 1.0f, 1.0f, Color{ 255, 255, 255, 255 }, group, active != 0);
			}
			break;
		}
		...
		case LEGACY_PANEL:
		{
			...
			AddPanel(id, posX, posY, width, height,
				Color{ static_cast<unsigned char>(r), static_cast<unsigned char>(g),
					static_cast<unsigned char>(b), static_cast<unsigned char>(a) },
		...
		case LEGACY_TEXTLABEL:
		{
			...
			const string text = ReadLegacyStringField(lineData);
			AddTextArea(id, m_Font.get(), text, posX, posY, 0, 0,
				Color{ static_cast<unsigned char>(r), static_cast<unsigned char>(g),
					static_cast<unsigned char>(b), static_cast<unsigned char>(a) },
				GuiTextArea::LEFT, group, active != 0);
			break;
